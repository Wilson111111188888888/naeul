...
import { TESTERS, type SkinType, type Tester } from "@/lib/testers";
import { Container } from "@/components/ui/container";
import { buttonClasses } from "@/components/ui/button";
import { SwipeCarousel } from "@/components/swipe-carousel";
import { cn } from "@/lib/utils";

function TesterCard({ t, className }: { t: Tester; className?: string }) {
  return (
    <li className={cn("flex flex-col rounded-2xl border border-line bg-cream p-6", className)}>
...
export function TestersWall({
  ...
  limit?: number;
  showFilters?: boolean;
  /** Aperçu en carrousel (swipe + points sur mobile) au lieu de la grille. */
  carousel?: boolean;
  className?: string;
}) {
    ...
    <section className={className}>
      <Container className="py-16 md:py-24">
        ...
        {carousel ? (
          <SwipeCarousel
            as="ul"
            className="mx-auto mt-10 flex max-w-5xl snap-x snap-mandatory gap-4 overflow-x-auto pb-2 scrollbar-hide md:grid md:grid-cols-3 md:gap-4 md:overflow-visible md:pb-0"
          >
            {shown.map((t, i) => (
              <TesterCard key={`${t.name}-${i}`} t={t} className="w-[85%] shrink-0 snap-center md:w-auto" />
            ))}
          </SwipeCarousel>
        ) : (
          <ul className="mx-auto mt-10 grid max-w-5xl gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {shown.map((t, i) => (
              <TesterCard key={`${t.name}-${i}`} t={t} />
            ))}
          </ul>
        )}
